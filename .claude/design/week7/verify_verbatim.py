"""Verify every quoted sentence in the week-7 plan/mockups is verbatim source.

Run from the repository root:  python .claude/design/week7/verify_verbatim.py
Exit code 0 = every quoted string matches docs/hpo/source/tyzden-7.md exactly.
...
PLAN = os.path.join(ROOT, ".claude", "design", "week7", "WEEK7_SCREEN_PLAN.md")
MOCKUPS = os.path.join(ROOT, ".claude", "design", "week7", "mockups", "*.html")
...
def norm(text):
    """Collapse whitespace and drop markdown so wrapping and quoting never lie."""
    # Blockquote markers are structure, not content: the plan quotes the author
    # inside a "> " block, and without this the markers land mid-sentence.
