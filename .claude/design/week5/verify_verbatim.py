"""Verify every quoted sentence in the week-5 plan/mockups is verbatim source.

Run from the repository root:  python .claude/design/week5/verify_verbatim.py
Exit code 0 = every quoted string matches docs/hpo/source/tyzden-5.md exactly.
...
# Day 2 shows the week-4 wording of STOP side by side with the week-5 one
# (OQ-2), so week 4 is authored copy on that page too.
SRC4 = os.path.join(ROOT, "docs", "hpo", "source", "tyzden-4.md")
PLAN = os.path.join(ROOT, ".claude", "design", "week5", "WEEK5_SCREEN_PLAN.md")
MOCKUPS = os.path.join(ROOT, ".claude", "design", "week5", "mockups", "*.html")
...
OURS = {
    "Vybrat si sám/sama",
    "toto sa teraz naučíš → toto teraz vyplníš",
    # Annotations and labels we wrote into the mockups.
    "Řetěz končí — další otázky se ptají jen tehdy, když je odpověď „ano“",
    "STOP už je v aplikaci z týdne 4, ale s jiným zněním písmen. Uživatel, "
    "který prošel týden 4, uvidí stejnou techniku podruhé jinak – to působí "
    "jako chyba aplikace. Níže obě verze; prosím vyberte jednu.",
    "Doporučujeme: je už v aplikaci i v uložených záchranných balíčcích "
    "uživatelů a je celá česky.",
    "Obsahuje navíc „doslova se nadechni“ u písmene T; anglická písmena drží "
    "akronym STOP přesněji.",
    # Draft intro for Day 4 (OQ-6) — marked NÁVRH in the mockup.
...
DEMO = {
    "Kamarádka mi neodepsala celý den, i když viděla zprávu.",
    "Měla jsem pocit, že to stejně nezvládnu.",
    "Hádka doma, večer po půlnoci, když jsem hladová.",
}
...
src4 = norm(io.open(SRC4, encoding="utf-8").read())
...
def check(label, candidate):
    ...
    if len(candidate) < MIN_CONTENT_LEN or ELLIPSIS in candidate:
        ...
        return
    checked += 1
    if candidate not in src and candidate not in src4:
